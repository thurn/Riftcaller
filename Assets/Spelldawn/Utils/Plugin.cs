  ...
  static class Plugin
  {
    ...
    public static CommandList? Connect(ConnectRequest request)
    {
      if (!_initialized)
      {
        ...
        Errors.CheckNonNegative(spelldawn_initialize(encoded, encoded.Length), "Plugin initialization error");
        _initialized = true;
      }
      ...
      var responseSize = Errors.CheckNonNegative(
          spelldawn_connect(input, input.Length, output, output.Length),
    ...
    public static CommandList PerformAction(GameRequest request)
    {
      ...
      var output = new byte[BufferSize];
      var responseSize = Errors.CheckNonNegative(
          spelldawn_perform_action(input, input.Length, output, output.Length),
          "Plugin action error");
